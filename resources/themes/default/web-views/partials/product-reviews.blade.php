        ...
        <div class="col-md-7">
            ...
            @if (!empty(json_decode($productReview->attachment)))
                @foreach (json_decode($productReview->attachment) as $key => $photo)
                    <img onclick="showInstaImage('{{asset("storage/review/$photo")}}')" class="cz-image-zoom __img-70 rounded border" onerror="this.src='{{asset('public/front-end/img/image-place-holder.png')}}'" src="{{asset("storage/review/$photo")}}" alt="Product review">
                @endforeach
            @endif
        </div>
